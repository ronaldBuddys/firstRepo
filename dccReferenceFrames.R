...
## make (dc) correlation reference frames using endpoints
dccRefFrames <- function(rar.norm = NULL,cor.decay = 500, on = "quarters", aggregate.over.days = 2,
						initial.cor.mat = NULL, prog.bar = TRUE)
{
	...
	ref.cor.mat <- lapply(1:(length(eps) - 1), function(i)
			{
				...
				output <- DCC(rar.norm[use.interval,], lambda = d.to.lambda(cor.decay), 
						initial.matrix = initial.cor.mat,
...
getDccMat <- function(ref.cor.mat = NULL, date = NULL, rar.norm = NULL, 
				usable.markets = NULL, 
...
#	start.date <- index(rar.norm)[which(index(rar.norm)%in%cm.date) + 2 - aggregate.over.days]
	...
	if(date%in%cm.date)
	{
		cor.mat <- initial.cor.mat[usable.markets, usable.markets]
	}
	if(!date%in%cm.date)
	{				
		cor.mat <- DCC(rar.norm[date.interval,usable.markets], showMsg = FALSE,
				lambda = d.to.lambda(cor.decay),
				initial.matrix = initial.cor.mat[usable.markets, usable.markets],
